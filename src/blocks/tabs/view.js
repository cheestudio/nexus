const macTabs = () => {
  const tabsContainers = document.querySelectorAll('.mac-tabs');

  tabsContainers.forEach((tabsContainer) => {
    // Find all tab items
    const tabContents = tabsContainer.querySelectorAll('.mac-tab-item');
    if (!tabContents.length) return;

    // Create tabs navigation if it doesn't exist
    const tabNavList = tabsContainer.querySelector('.mac-tabs__nav-list');
    if (!tabNavList.children.length) {
      ...
      tabContents.forEach((tabContent, index) => {
        ...
        tabItem.className = 'mac-tabs__nav-item';
        tabItem.setAttribute('role', 'presentation');
        
        const tabButton = document.createElement('button');
        tabButton.className = 'mac-tabs__nav-button';
    ...
    const tabButtons = tabNavList.querySelectorAll('.mac-tabs__nav-button');
  ...
  function activateTab(tabsContainer, activeIndex) {
    const tabButtons = tabsContainer.querySelectorAll('.mac-tabs__nav-button');
    const tabContents = tabsContainer.querySelectorAll('.mac-tab-item');
